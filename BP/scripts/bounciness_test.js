...
BOUNCINESS COMPONENT BY NaKeR_N
Tutorial:
Add component to the block;
Add necessary Tags: [
"ff:bouncy" - Marks the block as bouncy to check in some functions. Is NOT required BUT is highly recommended
"ff:bounciness_[your number here, may be float]" - Specify the bounciness of the block. This is a REQUIRED parameter;
"ff:bounciness.reduction_[your number here, may be float]" - Specify how much the impulse will be reduced. Uses division. Can be not specified - defaults to 2.
"ff:bounciness.noFallDamage" - this one does not require any numbers. This makes that falling on the block will not deal fall damage. Only for players.
]
*/
...
mc.world.beforeEvents.worldInitialize.subscribe(data => {
    data.blockTypeRegistry.registerCustomComponent(
        "ff:bounciness",
        bouncinessComponent
    )
})
