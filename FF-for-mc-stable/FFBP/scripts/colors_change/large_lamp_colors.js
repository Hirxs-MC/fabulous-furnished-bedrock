...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
    initEvent.blockComponentRegistry.registerCustomComponent('ff:large_lamp_function_on', {
        onPlayerInteract: e => {
            ...
            // white 
            block.dimension.runCommandAsync(`execute at @e[type=ff:ff_light_ray] positioned ${x} ${y} ${z} run event entity @e[r=0.5] destroy`);
        ...
        onTick: e => {
            const { player, block } = e;
            const { x, y, z } = block.location;
            block.dimension.spawnParticle("ff:ff_light_ray", { x: block.location.x + 0.5, y: block.location.y + 1.1, z: block.location.z + 0.5 });
        }
    });
});
...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
    initEvent.blockComponentRegistry.registerCustomComponent('ff:large_lamp_function_off', {
        onPlayerInteract: e => {
            ...
            const { x, y, z } = block.location;
            // white
            block.dimension.runCommandAsync(`summon ff:ff_light_ray ${x} ${y} ${z}`);
            if (block.permutation.getState("ef:colors") === 0 && block.permutation.getState("minecraft:cardinal_direction") === "north") {
